...
job = Job(glueContext)
s3_path = "s3://spotify-etl-project-srinalla/raw_data/to_processed/"
...
def process_albums(df):
    df_out = df.withColumn("items", explode("items")).select(
...
def process_artists(df):
    df_out = df.select(explode(col("items")).alias("item")) \
        .select(explode(col("item.track.artists")).alias("artist")) \
        .select(
            col("artist.id").alias("artist_id"), 
            col("artist.name").alias("artist_name"), 
            col("artist.href").alias("external_url")
        ) \
        .drop_duplicates(["artist_id"])
    return df_out

def process_songs(df):
    df_songs = df.select(explode(col("items")).alias("item")) \
        .select(
            col("item.track.id").alias("song_id"), 
            col("item.track.name").alias("song_name"), 
            col("item.track.duration_ms").alias("song_duration"),
...
def write_to_s3(df, path_suffix, format_type="csv"):
    ...
    glueContext.write_dynamic_frame.from_options(
        frame = dynamic_frame,
        connection_type = "s3",
        connection_options = {"path": f"s3://spotify-etl-project-srinalla/transformed_data/{path_suffix}/"},
        format = format_type
    )
